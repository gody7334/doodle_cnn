import _init_paths
import os
experiment = '002_mobilenet_224_1'
...
from net.customed_net.MobileNetV2 import *
...
def do_valid( net, valid_loader, criterion ):
    ...
    for input, truth, cache in valid_loader:
        ...
        # only use 1/100 data to val, otherwise, too much
        # if valid_num>len(input)*20:
        # if valid_num==len(valid_loader.sampler)/100:
            # break;
...
def run_train():
    ...
    fold    = 0
    out_dir = \
        '/home/gody7334/Project/tensorflow/ipython/doodle-quickdraw/data/result/'+experiment

    initial_checkpoint = \
        None
        # '/home/gody7334/Project/tensorflow/ipython/doodle-quickdraw/data/result/'+'000_resnet50'+'/checkpoint/00050000_model.pth'

    pretrain_file = \
        '/dataset/pytorch_models/mobilenet_v2.pth'
        # None

    # schduler  = NullScheduler(lr=0.01)
    schduler  = CyclicScheduler(min_lr=0.0001,max_lr = 0.001, period=1000)
    # schduler = DecayScheduler(base_lr=0.001, decay=0.9, step=100000, min_lr=0.0001)
    ...
    batch_size = 384  #16 #32

    train_dataset = DoodleDataset('train', 'train_df_'+str(fold), train_augment)
    ...
    valid_dataset = DoodleDataset('train', 'val_df_'+str(fold),  valid_augment)
    ...
    net = MobileNetV2(n_class=340).cuda()
    # net = Net().cuda()
    ...
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, net.parameters()),
                          lr=schduler(0), weight_decay=0.0001)



    num_iters   = int(50000000/batch_size) * 5  # (data size / batch size) = step then times epoch
    iter_smooth = 20
    iter_log    = 50
    iter_valid  = 200
